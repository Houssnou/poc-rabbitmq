...
1. Run docker-compose file to create an isntance of RabbitMQ on container:
   ...
   . type docker-compose up -d ( `-d` allows the command to be executed in detached mode freeing the terminal)
...
3. Add the following to each service:
  . Add the connection string settings to create the database (Banking and Transfer respectively for each service). 
  . Create the database:
	...
	. Ensure that the database are created successfully.
